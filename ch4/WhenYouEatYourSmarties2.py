...
# The red ones are the best, so I eat them slowly one at a time. The other smarties_colors I eat quickly by the handful (my hand can hold a maximum of 7 Smarties).
...
# I have a big box of Smarties. After I've finished sorting the smarties_colors, how long will it take me to eat them?
...
for datafile in range(10):
   ...
   # while not finish:
    ...
    # 시도 2: 무식 하지만 총 먹는 시간을 변수로 선언 해주고, 우선 빨강은 제외 시키기 위해 검사 한번 진행 그 이외 색을 가지고 구현.
    total_eat_time = 0

    if smarties_color != red_num: # 우선 컬러가 빨강 인지 검사 하고, 아니면 그 갯수가 7을 넘어 가는 상황 이면 13초 추가 해주는 코드 구현!
        if 8 <= (orange_num // 7 ) <= 13 :
            total_eat_time += 13

        if 15 <= (orange_num // 7 ) <= 20:
            total_eat_time += (13*2)

        if 22 <= (orange_num // 7) <= 27:
            total_eat_time += (13*3)

        # 이런 식으로 가면 무한 if 지옥에 걸림. -> 그럼 배운 모드 연산자를 쓰면 간단하게 될까?
